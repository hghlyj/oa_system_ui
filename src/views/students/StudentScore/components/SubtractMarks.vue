      ...
      <el-row :gutter="24">
        <el-col :span="3"
          ><el-input
            ...
            placeholder="请输入内容"
            @keyup.enter.native="GetSubtractMarks"
            label-width="50px"
          ></el-input
        ></el-col>
        <el-col :span="3"
...
export default {
  ...
  async created() {
    ...
    const res = await GetPutIntoEffect(3);
    if (res.code == 200) {
    }
  ...
  methods: {
    ...
    async handleUpdate(row) {
      this.resetTemp();
      const res = await GetSubtractMarksOne(row.id);
      if (!res) {
        this.$notify({
          title: "error",
          message: "获取单条数据失败",
      ...
      this.temp = {
        ...
        content: res.content,
        Marks: res.Marks,
        Disciplinetype: res.Disciplinetype,
        expel: res.expel,
        remark: res.remark,
      };
    ...
    updateData() {
      this.$refs["dataForm"].validate(async (valid) => {
        if (valid) {
          const res = await UpdateSubtractMarks(this.temp.id, this.temp);
          if (res) {
            this.GetSubtractMarks();
            this.dialogFormVisible = false;
            this.$notify({
    ...
    async handleDelete(row) {
      const res = await DeleteSubtractMarks(row.id);
      // if (res) {
        this.GetSubtractMarks();
        this.$notify({
          title: "Success",
