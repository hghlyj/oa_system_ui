    ...
    <el-tabs v-model="awaactiveName" @tab-click="awahandleClick">
      <el-row :gutter="24">
        ...
        <el-col :span="3"
          ...
          ></el-input
        ></el-col>
        <el-col :span="3" v-permission="['admin', 'dc']">
          <el-select
...
export default {
  ...
  async created() {
    ...
    if (roles.indexOf("fdy") != -1) {
      this.tratqueryInfo.query.fdy_id = id;
      this.fdyjs = false;
    } else if (roles.indexOf("js") != -1) {
      this.tratqueryInfo.query.js_id = id;
    }
  ...
  methods: {
    ...
    resetInput() {
      // const status = JSON.stringify(this.tratqueryInfo.query.status);
      // const status1 = JSON.parse(this.tratqueryInfo.query.status);
      const status = this.tratqueryInfo.query.status;
      const js_id = this.tratqueryInfo.query.js_id;
      const fdy_id = this.tratqueryInfo.query.fdy_id;
      this.tratqueryInfo = {
        query: {
          fdy_id: null,
          js_id: null,
      ...
      // this.tratqueryInfo.query.status = status1;
      this.tratqueryInfo.query.status = status;
      this.tratqueryInfo.query.js_id = js_id;
      this.tratqueryInfo.query.fdy_id = fdy_id;
    },
    //获取所有学院
