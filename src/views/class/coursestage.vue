<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 用户列表区域 -->
      <el-table :data="collegeStage" border stripe>
        <el-table-column type="index"></el-table-column>
        <el-table-column label="阶段序号" prop="id"></el-table-column>
        <el-table-column label="阶段名称" prop="name"></el-table-column>
    ...
    >
      <el-form
        ...
        label-width="70px"
      >
        <el-form-item label="阶段名称" prop="name" label-width="85px">
          <el-input v-model="addForm.name"></el-input>
        </el-form-item>
        <el-form-item label="所属学院" prop="college_id" label-width="85px">
          <el-select v-model="addForm.college_id" placeholder="请选择学院">
            <el-option
              v-for="item in collegesOptions"
              :key="item.value"
              :label="item.label"
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addCollegeStage">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
import { getCollegeStage, createCollegeStage } from "network/api/college_stage";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    async getCollegeStageList() {
      const result = await getCollegeStage(this.queryInfo);
      this.collegeStage = result.data; //列表中展示的数据内容
    ...
    addCollegeStage() {
      this.$refs.addFormRef.validate(async (valid) => {
        if (!valid) return;
        // 可以发起添加用户的网络请求
        if (await createCollegeStage(this.addForm)) {
