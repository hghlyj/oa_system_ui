...
export default {
  ...
  methods: {
    ...
    */
    async getDeparList(page = 1, page_size = this.queryInfo.pagesize) {
    ...
    handleSizeChange(newSize) {
      ...
      this.getDeparList(1, newSize);
    },
    // 监听 页码值 改变的事件
    handleCurrentChange(newPage) {
      this.getDeparList(newPage);
    },
    ...
    async addDepartment() {
      ...
      if (this.addForm.label == 1 || this.addForm.label == 0) {
        ...
        const result = await addDepartment(this.addForm);
        if (!result) {
    ...
    async editDepartment() {
      // 发起修改用户信息的数据请求
      const result = await editDepartmentItem(
        this.editForm.id,
        this.editForm.name,
        this.editForm.label
      );
